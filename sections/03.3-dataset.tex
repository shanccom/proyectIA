...
Para el desarrollo del presente proyecto se empleará el dataset \textit{Brain Tumor MRI Datasets}, publicado por Shashin en Kaggle \cite{kaggle_brain_tumor}. Dicho dataset contiene un total de 8764 imágenes de resonancia magnética (MRI) del cerebro organizadas para un problema de clasificación binaria.

Las imágenes se encuentran clasificadas en dos categorías:
\begin{itemize}
    \item \textbf{Yes}: pacientes con presencia de tumor cerebral.
    \item \textbf{No}: pacientes sin evidencia de tumor cerebral.
\end{itemize}
...
\begin{tabular}{lrrr}
\hline
\textbf{Conjunto} & \textbf{No Tumor} & \textbf{Tumor} & \textbf{Total} \\
\hline
Train     & 2869 & 4143 & 7012 \\
Test      &  717 & 1035 & 1752 \\
\textbf{Total} & 3586 & 5178 & \textbf{8764} \\
\hline
\end{tabular}
\end{table}
...
Como se observa en la Figura~\ref{fig:dist_clases}, el conjunto de datos presenta \textbf{5178 imágenes con tumor (59.1\%)} y \textbf{3586 imágenes sin tumor (40.9\%)}. Esto evidencia un ligero desbalance entre ambas clases, aunque la diferencia no es lo suficientemente grande como para impedir el entrenamiento del modelo.
...
La Figura~\ref{fig:dist_conjunto} muestra que la distribución de imágenes se mantiene tanto en el conjunto de entrenamiento como en el de prueba. En ambos casos, la clase \textbf{Tumor} posee una mayor cantidad de muestras que la clase \textbf{No Tumor}, conservando una proporción similar entre los dos subconjuntos.
